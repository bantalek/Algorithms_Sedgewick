...
const merge = (arr, low, mid, high, cb=e=>e) => {
...
const mergesort = (arr, low=0, high=arr.length - 1, cb=e=>e) => {
  if (low >= high) { // No more elements left to sort
    return;
  }

  let mid = Math.floor((low + high) / 2);
  mergesort(arr, low, mid, cb); // Sort the left half of the array
  mergesort(arr, mid + 1, high, cb); // Sort the right half of the array

  // Before executing a merge, observe if the two arrays are already sorted
  if (cb(arr[mid]) < cb(arr[mid + 1])) {
    return;
  }

  merge(arr, low, mid, high, cb); // Merge the results
